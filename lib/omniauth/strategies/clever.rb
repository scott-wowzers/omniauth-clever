    ...
    class Clever < OmniAuth::Strategies::OAuth2
      ...
      def callback_phase
        ...
        if error
          ...
          # On Windows we defer to the app
          elsif !request.params["skip_app_defer"] && (request.user_agent =~ /Windows/ || request.user_agent =~ /Mac/ || request.user_agent =~ /CrOS/ || request.user_agent =~ /Android/)
            call_app!
          else
            super
          end
